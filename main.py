...
with colA:
    st.subheader("Séries temporais (X e Y)")
    fig, ax = plt.subplots()
    ax.plot(t_s, x_r_s, label="X_real")
    ax.plot(t_s, x_m_s, label="X_modelo")
    ...
    fig, ax = plt.subplots()
    ax.plot(t_s, y_r_s, label="Y_real")
    ax.plot(t_s, y_m_s, label="Y_modelo")
    ax.set_xlabel(t_label)
    ax.set_ylabel("Y")
    ax.legend()
    st.pyplot(fig, clear_figure=True)
